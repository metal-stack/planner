...
import { fromObject, kv, note, toYaml, ymap, type YEntry, type YValue } from './yaml'
...
// exactly that module's `config`), one task per module, called directly
// in the deploy playbooks in the order the config builds up, `replaced` for what the plan fully describes and
...
/** The module calls, one task per module, tagged with their topic. */
export function enterpriseSonicTasks(): YValue[] {
  return TOPICS.flatMap(({ topic, label, steps }) =>
    steps.map((s) => task(topic, label, steps.length > 1, s)),
  )
}

/** What the modules don't cover, over SSH in config_db (as mini-lab does). */
export function enterpriseSonicSshTasks(): YValue[] {
  return [
    fromObject({
      name: 'DNS servers',
    ...
    toYaml(
      ymap(
        ...
        kv(
          'sonic_nameservers',
          nameservers,
          'Written to config_db over SSH by the Broadcom SONiC plays.',
        ),
      ),
    ),
  )
